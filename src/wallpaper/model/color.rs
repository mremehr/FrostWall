...
pub(crate) fn extract_palette_from_path(path: &Path) -> Result<(Vec<String>, Vec<f32>)> {
    let image = image::open(path).context("Failed to open image")?;
    extract_palette_from_image(&image)
}

pub(crate) fn extract_palette_from_image(image: &DynamicImage) -> Result<(Vec<String>, Vec<f32>)> {
    const K: usize = 5;
    ...
    let colors = color_weight_pairs
        .iter()
        .map(|(color, _)| color.clone())
        .collect();
    let color_weights = color_weight_pairs
        .iter()
        .map(|(_, weight)| *weight)
        .collect();
...
impl Wallpaper {
    ...
    pub fn extract_colors(&mut self) -> Result<()> {
        ...
        let (colors, color_weights) = extract_palette_from_path(&self.path)?;
        self.colors = colors;
